...
<div class="flex flex-col md:flex-row gap-8 p-4 w-full">
    ...
    <div class="md:w-1/2 flex flex-col gap-12 p-4 border rounded-lg shadow-sm">
        {{-- Logo --}}
        <div
            class="flex items-center justify-center border-2 border-dashed border-gray-300 rounded-2xl p-4 min-h-[150px] transition-opacity duration-300"
    ...
    <div class="md:w-1/2 p-4 border rounded-lg shadow-sm">
        <form wire:submit.prevent="saveVendor" class="space-y-6">
            ...
            <div>
                ...
                <input wire:model.defer="logo_url" type="url" id="logo_url" placeholder="Dán link logo để xem trước"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">
                @error('logo_url') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>
