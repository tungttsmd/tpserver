...
<div class="livewire-component-container">
    <form wire:submit.prevent="createSubmit" novalidate class="max-w-[480px] flex flex-col gap-4  border">
        <!-- Serial Number -->
        <x-atoms.wrappers.row-flex class="border justify-between">
            <x-atoms.form.input livewire-id="serial_number" form-id="serial_number" type="text"
                label="Serial number" placeholder="Nhập serial number" required="true" class="flex w-full justify-between"/>
        ...
        <!-- Phân loại -->
        <x-atoms.wrappers.row-flex class="border justify-between">
            <x-atoms.form.select livewireId="category_id" :collection="$categories" item-id="id" item-name="name"
                label="Phân loại" />
        </x-atoms.wrappers.row-flex>

        <!-- Bảo hành -->
        <x-atoms.wrappers.row-flex class="border gap-x-0">
            <x-atoms.form.checkbox form-id="has_warranty" label="Linh kiện có bảo hành" />
        </x-atoms.wrappers.row-flex>

        <x-atoms.wrappers.row-flex class="border justify-between">
            <div id="warranty-fields" class="flex flex-col w-full gap-2">
                <x-atoms.form.input livewire-id="warranty_start" form-id="warranty_start" type="date"
                    label="Ngày bắt đầu bảo hành" class="flex w-full justify-between"/>
                <x-atoms.form.input livewire-id="warranty_end" form-id="warranty_end" type="date"
                    label="Ngày kết thúc bảo hành" class="flex w-full justify-between"/>
            </div>
        </x-atoms.wrappers.row-flex>
